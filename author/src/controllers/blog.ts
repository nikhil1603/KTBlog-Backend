...
export const aiTitleResponse = TryCatch(async (req, res) => {
  ...
  const prompt = `Correct the grammar of the following blog title and return only the corrected title without any additional text: "${text}"`;
  ...
  try {
    ...
    if (!output) {
      res.status(400).json({ message: "No title generated" });
      return;
    }
...
export const aiBlogResponse = TryCatch(async (req, res) => {
  ...
  try {
    ...
    if (!rawText) {
      res.status(400).json({ message: "Empty AI response" });
      return;
    }
    ...
    const errorMessage =
      typeof error === "object" && error !== null && "message" in error
        ...
        : String(error);
    res.status(500).json({ message: "AI processing failed", error: errorMessage });
    return;
  }
});
